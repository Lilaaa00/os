...
int sys_process_run( int fd, int argc, const char **argv)
{
	...
	if (argc >= 2){
		...
		// The default priority for each process is 0.
		p->priority = 0;
	}
...
int sys_open_named_pipe(const char *fname)
{
	...
	struct namedpipe *p = namedpipe_open(fname);
	if(!p) {
		return KERROR_NOT_FOUND;
	}
	current->ktable[fd] = kobject_create_namedpipe(p);
	return fd;
}

int sys_make_named_pipe(const char *fname)
{
	return namedpipe_create(fname);
}
...
int32_t syscall_handler(syscall_t n, uint32_t a, uint32_t b, uint32_t c, uint32_t d, uint32_t e)
{
	...
	case SYSCALL_MAKE_NAMED_PIPE:
		return sys_make_named_pipe(a);
	case SYSCALL_OPEN_NAMED_PIPE:
		return sys_open_named_pipe(a);

...
struct kobject *kobject_create_namedpipe(struct namedpipe *p)
{
	struct kobject *k = kobject_create();
	k->type = KOBJECT_NAMEDPIPE;
	k->data.namedpipe = p;
	return k;
}
...
struct kobject * kobject_copy( struct kobject *ksrc )
{
	...
	case KOBJECT_PIPE:
		...
		break;
	case KOBJECT_NAMEDPIPE:
		namedpipe_addref(ksrc->data.namedpipe);
	}
...
int kobject_read(struct kobject *kobject, void *buffer, int size, kernel_io_flags_t flags )
{
	...
	case KOBJECT_NAMEDPIPE:
		if(flags&KERNEL_IO_NONBLOCK){
			actual = namedpipe_read_nonblock(kobject->data.pipe,buffer,size);
		}else{
			actual = namedpipe_read(kobject->data.namedpipe,buffer,size);
		}
		break;
	case KOBJECT_WINDOW:
		if(flags&KERNEL_IO_NONBLOCK) {
...
int kobject_write(struct kobject *kobject, void *buffer, int size, kernel_io_flags_t flags )
{
	...
	case KOBJECT_NAMEDPIPE:
		if(flags&KERNEL_IO_NONBLOCK) {
			return namedpipe_write_nonblock(kobject->data.namedpipe, buffer, size);
		} else {
			return namedpipe_write(kobject->data.namedpipe, buffer, size);
		}
	default:
		return 0;
	}
...
int kobject_close(struct kobject *kobject)
{
	...
	if(kobject->refcount==0) {
		...
		case KOBJECT_PIPE:
			...
			break;
		case KOBJECT_NAMEDPIPE:
			namedpipe_delete(kobject->data.namedpipe);
			break;
		default:
			break;
		}
		...
		if(kobject->type==KOBJECT_NAMEDPIPE){
			namedpipe_flush(kobject->data.namedpipe);
		}
	}
	return 0;
}
...
int kobject_size(struct kobject *kobject, int *dims, int n)
{
	...
	case KOBJECT_NAMEDPIPE:
		if(n==1) {
			dims[0] = namedpipe_size(kobject->data.pipe);
			return 0;
		} else {
			return KERROR_INVALID_REQUEST;
		}
